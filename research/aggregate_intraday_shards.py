...
from research.intraday_validation import walk_forward_trade_scorecards
...
def aggregate(archives: list[Path], output: Path) -> Path:
    manifests=[]; tables={name:[] for name in ("intraday_signals.csv","intraday_matched_controls.csv","intraday_market_conditions.csv")}; raw_trades=[]; stability_rows=[]; ranges=[]
    ...
    identities={(m["experiment"].get("hypothesis_id"),m["config"].get("strategy_names") and tuple(m["config"]["strategy_names"]),m["config"].get("cost_bps_per_side")) for m in manifests}
    ...
    with zipfile.ZipFile(output,"w",zipfile.ZIP_DEFLATED,compresslevel=1) as bundle:
        ...
        _write_csv_member(bundle,"intraday_walk_forward.csv",walk_forward); _write_csv_member(bundle,"intraday_scorecard.csv",scorecards)
