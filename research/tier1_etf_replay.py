...
from research.market_state_validation import (
    hierarchical_state_validation_outputs, state_validation_outputs,
)
...
from research.role_aware_evidence import build_role_aware_evidence
from research.isolated_strategy_diagnostics import (
    ...
    build_baseline_state_leaderboard, build_generation_candidate_map,
    build_defensive_baseline_comparisons, build_defensive_distinctness,
    build_locked_tactical_validation, iter_state_transition_timing,
...
def _cost_path_audit(scorecards: list[dict], trades: list[dict],
                     config: Tier1Config) -> list[dict]:
    """Explain cost-ladder path dependence instead of assuming monotonicity.

    Each cost level is a full independent portfolio simulation. Transaction
    costs can therefore move a later rebalance across the no-trade band. The
    audit records whether the sequence of trade decisions changed and flags a
    return increase at a higher cost for explicit review.
    """
    returns = {
        (row["strategy"], float(row["cost_bps"])): row["total_return"]
        for row in scorecards
    }
    decisions: dict[tuple[str, float], tuple[tuple[str, str, int], ...]] = {}
    for strategy in config.strategy_names:
        for cost in config.cost_ladder_bps:
            selected = [
                row for row in trades
                if row["strategy"] == strategy and float(row["cost_bps"]) == float(cost)
            ]
            decisions[(strategy, float(cost))] = tuple(
                (row["date"], row["symbol"], 1 if float(row["notional"]) > 0 else -1)
                for row in selected
            )
    rows = []
    for strategy in config.strategy_names:
        costs = sorted(float(value) for value in config.cost_ladder_bps)
        reference = decisions[(strategy, costs[0])]
        previous_return = None
        for cost in costs:
            total_return = returns[(strategy, cost)]
            path = decisions[(strategy, cost)]
            violation = (
                previous_return is not None and total_return > previous_return + 1e-12
            )
            rows.append({
                "strategy": strategy,
                "cost_bps": cost,
                "total_return": total_return,
                "trade_decisions": len(path),
                "decision_path_changed_vs_lowest_cost": path != reference,
                "return_increased_vs_lower_cost": violation,
                "cost_path_audit_required": violation or path != reference,
                "interpretation": (
                    "higher cost changed later no-trade-band decisions"
                    if path != reference else
                    "fixed trade-decision path across tested costs"
                ),
            })
            previous_return = total_return
    return rows
...
def _write_csv(bundle, name: str, rows):
    """Stream rows into ZIP so large diagnostics never exist twice in memory."""
    import io
    iterator = iter(rows)
    try:
        first = next(iterator)
        ...
        return
    with bundle.open(name, "w", force_zip64=True) as raw:
...
def run_tier1_job(job: dict, bars_path: Path, archive_path: Path, source_sha256: str, progress_callback=None) -> Path:
    ...
    state_labels,state_episodes,state_attribution,state_episode_returns=market_state_scorecards(
        all_daily,market_conditions,config.primary_cost_bps
    )
    (state_periods,state_inference,state_costs,state_transitions,state_folds,
     state_fold_recurrence,state_transition_horizons,state_survival)=state_validation_outputs(
    ...
    (hierarchy_periods, hierarchy_inference, hierarchy_costs,
     hierarchy_folds, hierarchy_recurrence, hierarchy_survival) = ([], [], [], [], [], [])
    if config.hierarchical_state_validation:
        (hierarchy_periods,hierarchy_inference,hierarchy_costs,hierarchy_folds,
         hierarchy_recurrence,hierarchy_survival)=hierarchical_state_validation_outputs(
            all_daily,market_conditions,config.cost_ladder_bps,config.primary_cost_bps,
            config.discovery_end_date,config.holdout_start_date,
            fold_sessions=config.walk_forward_test_sessions,
            progress_callback=progress_callback,
