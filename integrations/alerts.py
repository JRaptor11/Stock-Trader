...
def send_email_alert(subject, body):
    try:
        if app_state["utils"]["alerts"].get("email_suppressed"):
        ...
        if app_state["utils"]["alerts"].get("use_telegram", False):
        ...
        if e.smtp_code == 550 and b'daily user sending limit exceeded' in e.smtp_error.lower():
            ...
            app_state["utils"]["alerts"]["email_suppressed"] = True
            app_state["utils"]["alerts"]["email_suppression_reset"] = datetime.utcnow().date() + timedelta(days=1)
        else:
